...
本机保存不等于远端提交，也不会跨设备同步。重要反馈必须在 GitHub 新页面点提交后，才会进入 Issues 后台。如果后续反馈量增加，可以升级到飞书表格、GitHub Issues 工作流或 Supabase。
...
当前网站是 GitHub Pages 静态站，没有后端数据库。反馈先保存在浏览器本地，并支持复制或跳转 GitHub Issue；只有 GitHub Issue 页面提交成功后，后台才有记录。这样成本最低，也最稳定。
...
5. 反馈量变多后，接飞书表格、GitHub Issues 工作流或 Supabase，让反馈默认进入远端。
