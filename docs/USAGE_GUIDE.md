...
公司索引固定来自 32 家对象池，并分为必须覆盖、战略观察和实验对象。每张卡同时显示“活动状态 + 覆盖状态”、近 7/30 天合格事件、最近检查时间和主要接入缺口。点击公司后进入对象筛选；即使近期无事件，也能区分“确认安静、证据不足、接入失效或待接入”。
...
周报先把转载折叠为独立事实，再按主题聚合；至少两个独立事实才能形成本周主题。月报至少需要跨两个周次、三个独立事实，并与前一周期比较。事件分数只参与代表证据排序，不能替代独立性和跨周门槛。
