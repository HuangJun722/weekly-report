...
2. 经营 `data/entity_pool.json`。先从少量重点对象试点接入 Jobs、Changelog、Product Update，不要一次性铺开所有对象。
3. 为观察点补结构化转化监控：`raw_signal -> candidate_signal -> qualified_signal -> event -> window`。观察点不能只把内容塞进事件库，必须能统计过滤原因。
4. 周报/月报继续保留方向改造，但后置到对象信号更稳定之后。周报看窗口和方向，月报看趋势和结构变化，日报不再承担趋势裁决。
